...
def makeAvgFeatureVec(words, model, num_features):
    ''' Get one average of feature vector from one review (one row of data). '''
...
if __name__ == '__main__':
    ...
    train_df = pd.DataFrame(data=train_data_vec, index=train['id'])
    train_df.to_csv("../nlp_output/Word2Vec_Train_Features.csv", index=False, quoting=3)

    test_df = pd.DataFrame(data=test_data_vec, index=train['id'])
    test_df.to_csv("../nlp_output/Word2Vec_Test_Features.csv", index=False, quoting=3)
